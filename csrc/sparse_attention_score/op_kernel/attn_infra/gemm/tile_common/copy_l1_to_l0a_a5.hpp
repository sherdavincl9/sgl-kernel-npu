...
struct TileCopyTla<Arch::AtlasA5,
    ...
    __aicore__ inline void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor, uint32_t l0Batch)
    {
        ...
        for (uint32_t l0BatchIdx = 0; l0BatchIdx < l0Batch; l0BatchIdx++) {
            AscendC::LoadData(dstTensor.data()[l0BatchIdx * L0M * L0K], srcTensor.data()[l0BatchIdx * L1M * L1K],
                              loadDataParams);
        }
    }
};
...
struct TileCopyTla<Arch::AtlasA5,
    ...
    __aicore__ inline void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor)
    {
        ...
        if (L0M % ELE_NUM_PER_C0 == 0) {
            ...
            for (uint32_t kIdx = 0; kIdx < L0K / ELE_NUM_PER_C0; kIdx++) {
                ...
                auto dstOffset = dstTensor.layout()(dstTensor.coord());
                AscendC::LoadData(dstTensor.data()[dstOffset + kIdx * L0M * ELE_NUM_PER_C0], srcTensor.data(),
                                  loadDataParams);
            }
        }
    }
    ...
    __aicore__ inline void operator()(TensorDst const &dstTensor, TensorSrc const &srcTensor, uint32_t l0Batch)
    {
        ...
        if (L0M % ELE_NUM_PER_C0 == 0) {
            ...
            for (uint32_t l0BatchIdx = 0; l0BatchIdx < l0Batch; l0BatchIdx++) {
                for (uint32_t kIdx = 0; kIdx < L0K / ELE_NUM_PER_C0; kIdx++) {
                    AscendC::LoadData(dstTensor.data()[l0BatchIdx * L0M * L0K + kIdx * L0M * ELE_NUM_PER_C0],
                                      srcTensor.data()[l0BatchIdx * L1M * L1K + kIdx * ELE_NUM_PER_FRACTAL * 2],
                                      loadDataParams);
                }
            }
        }
