...
HOST_DEVICE constexpr auto apply(T &&t, F &&f, seq<I...>)
{
    return f(get<I>(static_cast<T &&>(t))...);
}

template <class T, class F, class G, int... I>
HOST_DEVICE constexpr auto tapply(T &&t, F &&f, G &&g, seq<I...>)
{
    return g(f(get<I>(static_cast<T &&>(t)))...);
}

template <class T0, class T1, class F, class G, int... I>
HOST_DEVICE constexpr auto tapply(T0 &&t0, T1 &&t1, F &&f, G &&g, seq<I...>)
{
...
HOST_DEVICE constexpr auto apply(T &&t, F &&f)
{
    return detail::apply(static_cast<T &&>(t), f, tuple_seq<T>{});
}

template <class T, class F, class G>
HOST_DEVICE constexpr auto transform_apply(T &&t, F &&f, G &&g)
{
...
HOST_DEVICE constexpr auto transform_apply(T0 &&t0, T1 &&t1, F &&f, G &&g)
{
...
template <class T, class F>
HOST_DEVICE constexpr void for_each(T &&t, F &&f)
{
    if constexpr (is_tuple<remove_cvref_t<T>>::value) {
...
struct UnpackedMakeTuple {
    template <class... T>
    HOST_DEVICE constexpr auto operator()(T const &...a) const
    {
...
HOST_DEVICE constexpr auto transform(T0 const &t0, T1 const &t1, F &&f)
{
...
HOST_DEVICE constexpr decltype(auto) get(T &&t) noexcept
{
...
HOST_DEVICE constexpr auto max(T0 const &t0, Ts const &...ts);

struct UnpackedMax {
    template <class... T>
    HOST_DEVICE constexpr auto operator()(T const &...v) const
    {
        return tla::max(v...);
    }
};
...
HOST_DEVICE constexpr auto rank(Tuple const &t)
{
...
// depth
template <int... Is, class Tuple>
HOST_DEVICE constexpr auto depth(Tuple const &t);
...
struct Product {
    ...
    HOST_DEVICE constexpr auto operator()(IntTuple const &a) const
    {
...
HOST_DEVICE constexpr auto Add(IntTupleA const &a, IntTupleB const &b);

struct UnpackedAdd {
    template <class IntTupleA, class IntTupleB>
    HOST_DEVICE constexpr auto operator()(IntTupleA const &x, IntTupleB const &y) const
    {
        return Add(x, y);
    }
};
