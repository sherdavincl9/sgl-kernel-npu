...
#include "catlass/gemm/tile/kda_tile_traits.hpp"
...
protected:
        ...
        uint32_t &bufferIndex)
    {
        ...
        for (uint32_t ldk = 0; ldk < kLoop; ldk++) {
            uint32_t kActual = (problemShape.k() < (ldk + 1) * params.splitkLength) ? kResidueLenght : params.splitkLength;
