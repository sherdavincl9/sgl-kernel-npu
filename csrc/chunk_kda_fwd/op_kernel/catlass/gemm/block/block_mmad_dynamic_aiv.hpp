        ...
        GemmCoord const &actualShape)
    {
        ...
        for (uint32_t i = 0; i < mTiles; ++i) {
            for (uint32_t j = 0; j < VNCHW_SIZE; ++j) {
                srcLocalList[j] = ubCTensor.GetPhyAddr() + i * C0_NUM_PER_FRACTAL * ubTileSize * sizeof(ElementC) + j * ubTileSize * sizeof(ElementC);
                if constexpr (std::is_same_v<ElementC, half>) {
                    dstLocalList[j] = ubTransposeCTensor.GetPhyAddr() + i * C0_NUM_PER_FRACTAL * sizeof(ElementC) + j * ubTileNum * sizeof(ElementC);
                }
                if constexpr (std::is_same_v<ElementC, float>) {
                    dstLocalList[j] = ubTransposeCTensor.GetPhyAddr() + i * C0_NUM_PER_FRACTAL * sizeof(ElementC) + j / 2 * ubTileNum * sizeof(ElementC) + j % 2 * BYTE_PER_C0;
                }
            }
            if (usedBufferNum > 1) {
