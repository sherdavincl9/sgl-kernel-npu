...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_DEQUANT_TLA_HPP
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_DEQUANT_TLA_HPP
...
template <
    ...
    class TileMmad_>
struct BlockMmadTla<
    MmadDequant<ArchTag_, ENABLE_UNIT_FLAG_, USE_HF32_MODE_, L0C_STAGES_, ENABLE_L1_RESIDENT_, L1A_STAGES_,
...
public:
    // Type Aliases
    using DispatchPolicy = MmadDequant<ArchTag_, ENABLE_UNIT_FLAG_, USE_HF32_MODE_, L0C_STAGES_, ENABLE_L1_RESIDENT_,
    ...
    static constexpr Tile::ScaleGranularity DEQUANT_MODE = TileCopy::DEQUANT_GRANULARITY;
    static constexpr bool HAS_QUANT_TENSOR = TileCopy::HAS_QUANT_TENSOR;
    ...
    static constexpr uint32_t L1AB_TILE_SIZE = L1A_TILE_SIZE * L1A_STAGES + L1B_TILE_SIZE * L1B_STAGES;
    // L0 tile size
    ...
    // Check L1TileShape
    static_assert(L1AB_TILE_SIZE <= ArchTag::L1_SIZE, "L1TileShape exceeding the L1 space!");
    ...
    static_assert((!HAS_BIAS && !HAS_QUANT_TENSOR && (L1A_STAGES + L1B_STAGES) <= 8) ||
                      (HAS_BIAS && !HAS_QUANT_TENSOR && (L1A_STAGES + L1B_STAGES) <= 7) ||
                      (HAS_QUANT_TENSOR && (L1A_STAGES + L1B_STAGES) <= 6),
    ...
    static constexpr auto L1QUANT_LAYOUT = tla::MakeLayout(tla::Int<L1_TILE_N>{});
    ...
    BlockMmadTla(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart = 0)
    {
        if ASCEND_IS_AIC {
            ...
            uint32_t l1QuantOffset = l1BiasOffset;
            if constexpr (HAS_BIAS) {
                l1QuantOffset += L1_TILE_N * sizeof(ElementBias);
            }
            // Init buffers
            ...
            if constexpr (HAS_QUANT_TENSOR) {
                l1QuantTensor = resource.l1Buf.template GetBufferByByte<uint64_t>(l1QuantOffset);
    ...
    template <class TensorA, class TensorB, class TensorC, class TensorBias = EmptyClass, class TensorQuant = EmptyClass>
    CATLASS_DEVICE void operator()(TensorA &tensorA, TensorB &tensorB, TensorC &tensorC, GemmCoord const &actualShape,
                                   TensorBias const &tensorBias = {}, TensorQuant const &tensorQuant = {}, float scaleParam = 1.0)
    {
        // Check TileShape
        if constexpr (HAS_BIAS) {
            static constexpr uint32_t BIAS_BUF_SIZE = L0_TILE_N * sizeof(ElementAccumulator);
            ...
            static_assert(L1AB_TILE_SIZE + L1BIAS_SIZE <= ArchTag::L1_SIZE,
                          "L1TileShape exceeding the L1 space!");
        }
        if constexpr (HAS_QUANT_TENSOR) {
            static constexpr uint32_t L1QUANT_SIZE = L1_TILE_N * sizeof(uint64_t);
            if constexpr (HAS_BIAS) {
                static constexpr uint32_t L1BIAS_SIZE = L1_TILE_N * sizeof(ElementBias);
                static_assert(L1AB_TILE_SIZE + L1BIAS_SIZE + L1QUANT_SIZE <= ArchTag::L1_SIZE,
                              "L1TileShape exceeding the L1 space!");
            } else {
                static_assert(L1AB_TILE_SIZE + L1QUANT_SIZE <= ArchTag::L1_SIZE,
                              "L1TileShape exceeding the L1 space!");
            }
        }
        ...
        if constexpr (ENABLE_L1_RESIDENT) {
            ...
            // skip this loading.
            if (lastAddrA[l1AListId] != tensorTileA.data().GetPhyAddr() || tla::get<0>(tensorTileA.coord()) != lastCoordA[l1AListId].row() || tla::get<1>(tensorTileA.coord()) != lastCoordA[l1AListId].column()) {
        ...
        if constexpr (ENABLE_L1_RESIDENT) {
            if (lastAddrB[l1BListId] != tensorTileB.data().GetPhyAddr() || tla::get<0>(tensorTileB.coord()) != lastCoordB[l1BListId].row() || tla::get<1>(tensorTileB.coord()) != lastCoordB[l1BListId].column()) {
        ...
        for (uint32_t kL1Idx = 0; kL1Idx < kL1Loop; kL1Idx++) {
            ...
            if (kL1Idx < kL1Loop - 1) {
                ...
                if constexpr (ENABLE_L1_RESIDENT) {
                    if (lastAddrA[l1AListIdNext] != tensorTileA.data().GetPhyAddr() || tla::get<0>(tensorTileA.coord()) != lastCoordA[l1AListIdNext].row() || tla::get<1>(tensorTileA.coord()) != lastCoordA[l1AListIdNext].column()) {
                        copyGmToL1A(tensorL1A, tensorTileA);
                ...
                if constexpr (ENABLE_L1_RESIDENT) {
                    if (lastAddrB[l1BListIdNext] != tensorTileB.data().GetPhyAddr() || tla::get<0>(tensorTileB.coord()) != lastCoordB[l1BListIdNext].row() || tla::get<1>(tensorTileB.coord()) != lastCoordB[l1BListIdNext].column()) {
                        copyGmToL1B(tensorL1B, tensorTileB);
        ...
        // copy block out
        if constexpr (DEQUANT_MODE == Tile::ScaleGranularity::PER_TENSOR) {
            copyL0CToDst.params.scale = scaleParam;
        }

        if constexpr (HAS_QUANT_TENSOR && !std::is_same_v<TensorQuant, EmptyClass>) {
            using CopyGmToL1Scale = typename TileCopy::template CopyGmToL1Scale<TensorQuant>;
            CopyGmToL1Scale copyGmToL1Scale;
            auto tensorL1Quant = tla::MakeTensor(l1QuantTensor, L1QUANT_LAYOUT, Arch::PositionL1{});
            AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(QUANT_EVENT_ID);
            copyGmToL1Scale(tensorL1Quant, tensorQuant);
            ...
            if constexpr (!ENABLE_UNIT_FLAG) {
                ...
                copyL0CToDst(tensorC, tensorL0C, tensorL1Quant);
                AscendC::SetFlag<AscendC::HardEvent::FIX_M>(l0CEventList[l0CListId]);
                l0CListId = (l0CListId + 1 < L0C_STAGES) ? (l0CListId + 1) : 0;
            } else {
                copyL0CToDst(tensorC, tensorL0C, tensorL1Quant, 0b11);
            }
            AscendC::SetFlag<AscendC::HardEvent::FIX_MTE2>(QUANT_EVENT_ID);
        } else {
            if constexpr (!ENABLE_UNIT_FLAG) {
...
protected:
    ...
    AscendC::LocalTensor<uint64_t> l1QuantTensor;
    ...
    static constexpr int32_t QUANT_EVENT_ID = L1A_STAGES + L1B_STAGES + 1;
...
#endif // CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_DEQUANT_TLA_HPP
