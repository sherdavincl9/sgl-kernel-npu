...
struct GetMatmulConfig<MatMulType::MM_PA> {
    static constexpr MatmulConfig mmcfg_value =
        GetNormalConfig(false, false, false, BatchMode::BATCH_LESS_THAN_L1, false);
    static constexpr bool ibshare_value = false;
};
...
public:
    ...
    static __aicore__ void CopyND2NZ(const LocalTensor<SRC_T> &dst, const GlobalTensor<SRC_T> &src, const int row,
                                     ...
                                     const int dstNzMatrixStride = 1, const bool kAlignToC0Size = false,
                                     const int dstNzC0Stride = 0)
    {  // The minimum range of parameter values is 1.
    ...
    static __aicore__ void bmm1CopyB1Sparse(const LocalTensor<int8_t> &bMatrix, const __gm__ void *gm, int row, int col,
    ...
    template <bool isLayoutBSH>
    static __aicore__ void bmm2CopyB1Sparse(const LocalTensor<int8_t> &bMatrix, const __gm__ void *gm, int row, int col,
                                            int useK, int useN, const uint64_t tilingPtr, const uint64_t dataPtr)
    {
        GlobalTensor<uint32_t> bmmLocalInfo;
    ...
    using PACBmm1 = Matmul<a1Type, b1Type, c1Type, bias1Type, BSAT::mmCFG,
                           matmul::MatmulCallBackFunc<nullptr, nullptr, bmm1CopyB1Sparse<isBSH>>>;
    ...
    using PACBmm2 = Matmul<a2Type, b2Type, c2Type, bias2Type, BSAT::mmCFG,
                           matmul::MatmulCallBackFunc<nullptr, nullptr, bmm2CopyB1Sparse<isBSH>>>;
    PACBmm2 bmm2;
...
protected:
    ...
    __aicore__ inline void DataCopyTransposeOutBSH(LocalTensor<computeType> &bmm2ResUb)
    {
        ...
        if constexpr (BSAT::calcMode == Mode::HighPrecision || IsSameType<T, bfloat16_t>::value) {
            ...
            DataCopyTranspose2<O>(attentionOutGm, FinalResUb[nextTokensOffset * this->headSize],
                                  CopyTransposeType::TRANSPOSE_ND_UB_GM, transposeParams, transposeTilingData22,
                                  multiSeqOffset);
        } else {
            // Before copyOut, synchronous calculation.
            SetFlag<HardEvent::V_MTE3>(attenOutCopyOut);
    ...
    __aicore__ inline int64_t CalMultiSeqOffset(int sIdx)
    {
        ...
        if constexpr (BSAT::layout == BSALayout::BNSD) {
            multiSeqOffset =
                (int64_t)sIdx * this->tilingData->promptAttentionBaseParams.seqSize * (int64_t)this->MultiHeadQ +
                queryLeftpaddingSize * (int64_t)this->headSize;  // BNSD
        } else {
            multiSeqOffset =
                (int64_t)sIdx * this->tilingData->promptAttentionBaseParams.seqSize * (int64_t)this->MultiHeadQ +
        ...
        if (this->tilingData->promptAttentionBaseParams.isBSNDOut) {
            this->tailParams->multiSeqOffsetBSNDOut = (int64_t)sIdx *
                                                          ...
                                                          (int64_t)this->MultiHeadQ +
                                                      queryLeftpaddingSize * (int64_t)this->MultiHeadQ;
        }
    ...
    __aicore__ inline TYPENAME_BSH_VOID LoopSOuterOffsetInit(int64_t seqListOffsetSize, int sIdx)
    {
        ...
        tensorACoreOffset = (int64_t)seqListOffsetSize + (int64_t)this->tailParams->sOuterOffset * (int64_t)MultiHeadQ +
        ...
        if (this->tilingData->promptAttentionBaseParams.isKVHasLeftPadding) {
            seqInnerOffsetSize =
                ((int64_t)sIdx * (int64_t)tilingData->promptAttentionBaseParams.seqInnerSize + kvLeftPaddingSize) *
                (int64_t)MultiHeadKV;
            ...
            seqInnerOffsetSize =
                tilingData->promptAttentionBaseParams.seqSize == tilingData->promptAttentionBaseParams.seqInnerSize
                    ? (seqListOffsetSize - (queryLeftpaddingSize * (int64_t)MultiHeadQ)) / headNumRatio
                    : (int64_t)sIdx * (int64_t)tilingData->promptAttentionBaseParams.seqInnerSize *
                          (int64_t)MultiHeadKV;
        } else {
        ...
        tensorBCoreOffset =
            (int64_t)seqInnerOffsetSize + this->tailParams->batchNOffset / headNumRatio * this->headSize;
    }

    TEMPLATE_LAYOUT
    __aicore__ inline TYPENAME_BNSD_VOID LoopSOuterOffsetInit(int64_t seqListOffsetSize, int sIdx)
    {
        uint64_t head_stride_q = this->headSize * tilingData->promptAttentionBaseParams.seqSize;
        ...
        if (this->tilingData->promptAttentionBaseParams.isKVHasLeftPadding) {
            seqInnerOffsetSize = (int64_t)sIdx * (int64_t)this->tilingData->promptAttentionBaseParams.seqInnerSize *
                                     (int64_t)MultiHeadKV +
                                 (int64_t)kvLeftPaddingSize * (int64_t)this->headSize;
        } else if (this->isKvContinuous == 1) {
            seqInnerOffsetSize =
                tilingData->promptAttentionBaseParams.seqSize == tilingData->promptAttentionBaseParams.seqInnerSize
                    ? (seqListOffsetSize - (queryLeftpaddingSize * (int64_t)seq_stride)) / (int64_t)headNumRatio
                    : (int64_t)sIdx * (int64_t)head_stride_kv *
    ...
    LocalTensor<computeType> &bmm2ResPreUb, LocalTensor<float> &softmaxSumUb)
{
    ...
    if (remain) {
        Div(bmm2DivUb[loop * REPEAT_DATA_NUM], bmm2DivUb[loop * REPEAT_DATA_NUM], softmaxSumUb, remain,
            params->singleProcessSOuterSize, repeatParams);
    }
    if constexpr (!IsSameType<computeType, float>::value) {
        pipe_barrier(PIPE_V);
...
__aicore__ inline void AdaBlockSparseAttentionS1s2Bns1X910Base<BSAT>::UpdateVmul(LocalTensor<computeType> &softmaxExpUb)
{
    ...
    repeatParams.src1RepStride = (this->headSize + softmaxTypeByteNum - 1) / softmaxTypeByteNum;
    repeatParams.dstRepStride = (this->headSize + softmaxTypeByteNum - 1) / softmaxTypeByteNum;
    ...
    for (int i = 0; i < loop; i++) {
        Mul(bmm2ResPreUb[i * numOneRep], softmaxExpUb, bmm2ResPreUb[i * numOneRep], numOneRep,
            this->headParams->singleProcessSOuterSize, repeatParams);
    }
    if (remain) {
...
__aicore__ inline void AdaBlockSparseAttentionS1s2Bns1X910Base<BSAT>::GetSingleCoreParam(int sIdx)
{
    ...
    if (isActualLenDimsNull) {
        ...
        actualSeqLengthPerBatch = (tilingData->promptAttentionBaseParams.actualSeqLengthsSize == 1)
                                      ? actualSeqLengthsGm.GetValue(0)
    ...
    if (isActualLenDimsKVNull) {
        ...
        actualSeqLengthKVPerBatch = (tilingData->promptAttentionBaseParams.actualSeqLengthsKVSize == 1)
