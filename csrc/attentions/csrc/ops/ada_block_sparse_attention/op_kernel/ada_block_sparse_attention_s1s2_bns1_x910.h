...
protected:
    ...
    static __aicore__ inline void Mask2Pos(const LocalTensor<int32_t> &sparseId, const LocalTensor<int32_t> &index,
                                           const LocalTensor<int8_t> &sparseMask, uint32_t count,
                                           LocalTensor<uint8_t> &tmpUb)
    {
        ...
        AscendC::Sort<float, true>(sortedLocal, maskFloat, index.template ReinterpretCast<uint32_t>(), sortTmpLocal,
                                   alignCount / 32);
        pipe_barrier(PIPE_V);
    ...
    SoftmaxCompute(const LocalTensor<computeType> &mmResUb, const LocalTensor<float> &softmaxMaxUb,
                   const LocalTensor<float> &softmaxSumUb, const LocalTensor<computeType> &softmaxExpMaxUb,
                   const LocalTensor<uint8_t> &tmpUb, uint32_t souterSize, uint32_t colCount, uint32_t actColCount)
    {
        ...
        SoftmaxFlashV2<computeType, nonFirst, true, false>(mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb,
                                                           softmaxExpMaxUb, softmaxSumUb, softmaxMaxUb, tmpUb,
    ...
    __aicore__ inline void Res1VecCompute(BSAComputeParam *params)
    {
        ...
        if (this->softmaxSouterStepLen != this->softmaxFlashTilingData.srcM) {
            SoftMaxShapeInfo softmaxShapeInfo{
                ...
                static_cast<uint32_t>(this->softmaxSouterStepLen), static_cast<uint32_t>(actColCount)};
            constexpr uint32_t tmpBuffSize =
                sizeof(computeType) == 2 ? BSA_TEMP_BUFFER_SIZE_BYTE : BSA_TEMP_BUFFER_SIZE_BYTE / 2;
            this->softmaxFlashTilingDataNew = SoftMaxFlashV2TilingFunc(softmaxShapeInfo, sizeof(computeType),
                                                                       sizeof(float), tmpBuffSize, true, false);
        }
        auto maskValue =
            static_cast<computeType>(sizeof(computeType) == sizeof(float) ? BOOL_ATTEN_MASK_SCALAR_VALUE : -10000.0f);
        ...
        for (int64_t souterOffset = 0, mm1ResGmOffset = 0, nextSouterOffset = 0; souterOffset < singleProcessSOuterSize;
            ...
            if constexpr (sizeof(computeType) == 2) {
                DataCopy(this->bmm1ResGmDb[gmPingpong][mm1ResGmOffset], this->mmResUb[ubPingpong],
                         souterSize * colCount);
            } else {
    ...
    __aicore__ inline void Bmm1VecInputCopyIn()
    {
        ...
        if (this->softmaxFlashTilingData.srcK != softmaxColCount) {
            ...
            this->softmaxSouterStepLen =
                ((minSoftmaxSouterStepLen > params->singleProcessSOuterSize) || (minSoftmaxSouterStepLen == 0)
                     ? params->singleProcessSOuterSize
                     : minSoftmaxSouterStepLen);
        }
        WaitFlag<HardEvent::MTE3_MTE2>(this->bmm1ResCopyOutEvent[0]);
        DataCopy(this->mmResUb[0], this->bmm1ResGmDb[params->gmPingpong], this->softmaxSouterStepLen * softmaxColCount);
                                                                                 ...
                                                                                 int32_t lastSparseBlockId)
{
    ...
    for (auto sInnerLoopIdx = 0; sInnerLoopIdx < innerLoopTimes; sInnerLoopIdx++) {
        ...
        params->singleProcessSInnerBmmTail =
            params->isLastInnerIter ? computeSize - sInnerLoopIdx * basicSInnerSize : basicSInnerSize;
        ...
        auto copyOnceBlockCount =
            (params->singleProcessSInnerBmmTail + this->sparseBlockSize - 1) / this->sparseBlockSize;
        // * mm datacopy  from pos.getvalue(sparseMaskPosOffset)
                                                                           ...
                                                                           uint32_t bid, uint32_t nid)
{
    ...
    if ((pos >= 0) && (pos < n)) {
        taskId = this->taskQid.GetValue(allocatedQtaskAllCore + pos);
    }
                                                                              ...
                                                                              uint32_t sOuterBlockNum)
{
    ...
    pipe_barrier(PIPE_V);
    AscendC::Sort<half, true>(sortedLocal, costUb, index.template ReinterpretCast<uint32_t>(), sortTmpLocal,
                              colCount / 32);
    pipe_barrier(PIPE_V);
    ...
    for (uint32_t loopNIdx = 0; loopNIdx < this->headNumSize; loopNIdx++) {
        ...
        for (int bIdx = 0; bIdx < this->batchSize; bIdx++) {
            ...
            while (allocatedQtaskAllCore < sOuterBlockNum) {
                ...
                SetFlag<HardEvent::S_V>(eid_vs);
                WaitFlag<HardEvent::S_V>(eid_vs);
                if (sOuterLoopIdx < 0) {
                    continue;
                }

                params->singleProcessSOuterSize =
                    sOuterLoopIdx == sOuterBlockNum - 1 ? this->singleProcessSOuterSizeTail : sOuterSize;
                                                                                        ...
                                                                                        int32_t blockNum)
{
    ...
    this->softmaxResGmDb[0].SetGlobalBuffer(
        (__gm__ MM_IN_T *)this->workspaceGm[blockNum * this->spmTmpSize + coreIdx * mm1ResSize * reuseWorkspaceRatio]
            .GetPhyAddr());
    this->softmaxResGmDb[1].SetGlobalBuffer(
        (__gm__ MM_IN_T *)this
            ->workspaceGm[blockNum * this->spmTmpSize + coreIdx * mm1ResSize * reuseWorkspaceRatio + mm1ResSize]
            .GetPhyAddr());
