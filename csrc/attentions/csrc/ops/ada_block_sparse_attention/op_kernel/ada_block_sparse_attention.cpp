...
#define INVOKE_BSA_GENERAL_OP_IMPL(templateClass, ...)                                                               \
    ...
    do {                                                                                                             \
        ...
        op.Init(query, key, value, sparseMask, sparseCntTable, pseShift, attenMask, actualSeqLengths,                \
                actualSeqLengthsKV, blocktable, queryPaddingSize, kvPaddingSize, keySharedPrefix, valueSharedPrefix, \
                actualSharedPrefixLen, attentionOut, softmaxLse, user, tiling_data, tiling, &tPipe);                 \
        op.Process();                                                                                                \
    } while (0)
...
extern "C" __global__ __aicore__ void ada_block_sparse_attention_FIAS(
    ...
    __gm__ uint8_t *value_antiquant_offset, __gm__ uint8_t *keySharedPrefix, __gm__ uint8_t *valueSharedPrefix,
