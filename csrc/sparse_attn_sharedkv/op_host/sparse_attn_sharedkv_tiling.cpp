...
#define OP_LOGE(opName, fmt, ...) ((void)0)
#define OP_LOGI(opName, fmt, ...) ((void)0)
#define OPS_REPORT_VECTOR_INNER_ERR(opName, message) ((void)0)
#define OP_CHECK_IF(condition, logExpr, action) \
    do {                                        \
        if (condition) {                        \
            logExpr;                            \
            action;                             \
...
ge::graphStatus SASInfoParser::CheckRequiredInOutExistence() const
{
    ...
    if (kvLayout_ == SASLayout::PA_ND) {
        OP_CHECK_IF(opParamInfo_.oriBlockTable.tensor == nullptr,
                    OP_LOGE(opName_, "tensor of ori_block_table is nullptr"), return ge::GRAPH_FAILED);
    }
        ...
        if (perfMode_ == SASTemplateMode::CFA_TEMPLATE_MODE || perfMode_ == SASTemplateMode::SCFA_TEMPLATE_MODE) {
            if (kvLayout_ == SASLayout::TND && opParamInfo_.cuSeqLensCmpKv.tensor == nullptr) {
                OP_LOGE(opName_, "the layout_kv is %s, seqlens_cmp_kv must be provided.",
                        SASLayoutToSerialString(kvLayout_).c_str());
                return ge::GRAPH_FAILED;
            }
        }
        return ge::GRAPH_SUCCESS;
...
ge::graphStatus SASInfoParser::GetQueryAndOutLayout()
{
    ...
    if (qLayout_ == SASLayout::BSND) {
        OP_CHECK_IF(opParamInfo_.cuSeqLensQ.tensor != nullptr,
                    OP_LOGE(opName_, "when q's layout is BSND, cu_seqlens_q is null."), return ge::GRAPH_FAILED);
    }
    if (qLayout_ == SASLayout::TND) {
        OP_CHECK_IF(opParamInfo_.seqUsedQ.tensor != nullptr,
                    OP_LOGE(opName_, "when q's layout is TND, seqused_q is null."), return ge::GRAPH_FAILED);
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SASInfoParser::GetN2Size()
{
    ...
    if (opParamInfo_.cmpKv.tensor != nullptr) {
        ...
        if (perfMode_ == SASTemplateMode::SCFA_TEMPLATE_MODE) {
            ...
            OP_CHECK_IF(
                cmpKvN2Size_ != n2Size_ || n2Size_ != cmpSparseIndicesN2Size_,
                OP_LOGE(opName_, "N2 size check failed! Expected ori_kv's N2(%u) == cmp_sparse_indices's N2(%u).",
                        n2Size_, cmpSparseIndicesN2Size_),
                return ge::GRAPH_FAILED);
        }
        OP_CHECK_IF(cmpKvN2Size_ != n2Size_,
                    OP_LOGE(opName_, "N2 size check failed! Expected cmp_kv's N2(%u) ==ori_kv's N2(%u).", cmpKvN2Size_,
                            n2Size_),
                    return ge::GRAPH_FAILED);
...
ge::graphStatus SASInfoParser::GetActualSeqLenSize(uint32_t &size, const gert::Tensor *tensor, SASLayout &layout,
                                                   const std::string &name) const
{
    if ((tensor == nullptr)) {
        OP_LOGE(opName_, "when layout of q is %s, %s must be provided.", SASLayoutToSerialString(layout).c_str(),
                name.c_str());
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus SASInfoParser::GetKVTSize()
{
    ...
    // actual_seq_lens_ori_kv determines the B-axis size. The current API provides only oriSeq, so derive cmpSeq from
    // it.
...
ge::graphStatus SASInfoParser::GetMaxBlockNumPerBatch()
{
    if (opParamInfo_.oriBlockTable.tensor == nullptr) {
        OP_LOGE(opName_, "the layout_kv is %s, block_table must be provided.",
                SASLayoutToSerialString(kvLayout_).c_str());
        return ge::GRAPH_FAILED;
    }
    ...
    if (opParamInfo_.oriBlockTable.tensor->GetStorageShape().GetDim(1) < 0) {
        OP_LOGE(opName_, "%s's second dimension(%lld) should be non-negative number.", ORI_BLOCK_TABLE_NAME.c_str(),
    ...
    if (opParamInfo_.cmpBlockTable.tensor != nullptr) {
        ...
        if (opParamInfo_.cmpBlockTable.tensor->GetStorageShape().GetDim(1) <= 0) {
            OP_LOGE(opName_, "%s's second dimension(%lld) should be greater than 0", CMP_BLOCK_TABLE_NAME.c_str(),
...
ge::graphStatus SASInfoParser::GetBlockSize()
{
    ...
    cmpBlockSize_ = opParamInfo_.cmpKv.tensor == nullptr ? 0 : GetAxisNum(cmpKvShape_, SASAxis::Bs, kvLayout_);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SASInfoParser::GetS2SizeForTND()
{
    if (opParamInfo_.cuSeqLensKv.tensor == nullptr) {
        OP_LOGE(opName_, "the layout_kv is %s, seqlens_ori_kv must be provided.",
                SASLayoutToSerialString(kvLayout_).c_str());
        return ge::GRAPH_FAILED;
    }
    // if (opParamInfo_.sequsedKv.tensor == nullptr) {
    //     OP_LOGE(opName_, "the layout_kv is %s, sequsedKv must be provided.",
    //     SASLayoutToSerialString(kvLayout_).c_str()); return ge::GRAPH_FAILED;
    // }
...
ge::graphStatus SASInfoParser::GetActualseqInfo()
{
    ...
    if (qLayout_ == SASLayout::TND) {
        if (opParamInfo_.cuSeqLensQ.tensor != nullptr) {
            ...
            OP_CHECK_IF(actualLenDimsQ_ == 0, OP_LOGE(opName_, "cu_seqlens_q cannot be empty tensor."),
                        return ge::GRAPH_FAILED);
        } else {
        ...
        if (opParamInfo_.sequsedKv.tensor != nullptr) {
            if (qLayout_ == SASLayout::BSND) {
                if (opParamInfo_.sequsedKv.tensor->GetShapeSize() != bSize_) {
                    OP_LOGE(opName_, "seqused_kv's dimension should be equal to %u, but got %ld.", bSize_,
                            opParamInfo_.sequsedKv.tensor->GetShapeSize());
                    return ge::GRAPH_FAILED;
                }
            } else {
            ...
            OP_CHECK_IF(opParamInfo_.sequsedKv.desc->GetDataType() != ge::DT_INT32,
                        OP_LOGE(opName_, "seqused_kv's dtype must be DT_INT32."), return ge::GRAPH_FAILED);
        ...
        if (opParamInfo_.cuSeqLensKv.tensor != nullptr) {
            if (qLayout_ == SASLayout::BSND) {
                ...
                if (opParamInfo_.cuSeqLensKv.tensor->GetShapeSize() != (bSize_)) {
                    OP_LOGE(opName_, "cuSeqLensKv's dimension should be equal to %u, but got %ld.", bSize_,
                            opParamInfo_.sequsedKv.tensor->GetShapeSize());
                    return ge::GRAPH_FAILED;
                }
            }
...
ge::graphStatus SASInfoParser::Parse(SASTilingInfo &sasInfo)
{
    ...
    if (ge::GRAPH_SUCCESS != GetOpName() || ge::GRAPH_SUCCESS != GetNpuInfo() || ge::GRAPH_SUCCESS != GetOpParaInfo() ||
        ge::GRAPH_SUCCESS != GetKvLayout() || ge::GRAPH_SUCCESS != CheckRequiredParaExistence() ||
    ...
    if (ge::GRAPH_SUCCESS != GetN1Size() || ge::GRAPH_SUCCESS != GetN2Size() || ge::GRAPH_SUCCESS != GetGSize() ||
        ...
        ge::GRAPH_SUCCESS != GetS1Size() || ge::GRAPH_SUCCESS != GetS2Size() || ge::GRAPH_SUCCESS != GetQHeadDim() ||
                                           ...
                                           const SASLayout &actualLayout, const std::string &name) const
{
    ...
    OP_LOGE(opName_, "Tensor %s only supports layout %s, but got %s", name.c_str(), oss.str().c_str(),
            SASLayoutToSerialString(actualLayout).c_str());
}
...
template <typename T>
void SASTilingCheck::LogErrorNumberSupport(const std::vector<T> &expectNumberList, const T &actualValue,
...
template <typename T>
void SASTilingCheck::LogErrorDimNumSupport(const std::vector<T> &expectNumberList, const T &actualValue,
                                           const std::string &name) const
{
...
ge::graphStatus SASTilingCheck::CheckDimNumInLayoutSupport(const SASLayout &layout, const gert::StorageShape *shape,
                                                           const std::string &name) const
{
    const auto &dimIt = SAS_LAYOUT_DIM_MAP.find(layout);
...
ge::graphStatus SASTilingCheck::CheckSingleParaOriSparseIndices() const
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(opName_, "ori_sparse_indices N2(%u) should equal kv head num(%u).", oriSparseIndicesN2Size, n2Size_),
        return ge::GRAPH_FAILED);
    OP_CHECK_IF(
        oriSparseIndexWidth_ == 0 || oriSparseIndexWidth_ > SPARSE_LIMIT || (oriSparseIndexWidth_ % 128U) != 0U,
...
ge::graphStatus SASTilingCheck::CheckSingleParaCmpSparseIndices() const
{
    if (sasInfo_.perfMode == optiling::SASTemplateMode::SCFA_TEMPLATE_MODE) {
        ...
        if (cmpSparseIndicesLayout_ == SASLayout::TND) {
            ...
            OP_CHECK_IF(opParamInfo_.cmpSparseIndices.tensor->GetStorageShape().GetDim(DIM_NUM_FOUR - 1) != 512 &&
                            opParamInfo_.cmpSparseIndices.tensor->GetStorageShape().GetDim(DIM_NUM_FOUR - 1) != 1024,
                        OP_LOGE(opName_, "K should be 512 or 1024, but got: %lld ",
...
ge::graphStatus SASTilingCheck::CheckSingleParaOriBlockTable() const
{
    ...
    OP_CHECK_IF(opParamInfo_.oriBlockTable.tensor->GetStorageShape().GetShapeSize() == 0,
                OP_LOGE(opName_, "ori_block_table cannot be empty tensor."), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        (oriBlockSize_ <= 0 || oriBlockSize_ > BLOCK_SIZE_LIMIT || (static_cast<uint64_t>(oriBlockSize_) % 16 != 0UL)),
...
ge::graphStatus SASTilingCheck::CheckSingleParaCmpRatio() const
{
    if (sasInfo_.perfMode == optiling::SASTemplateMode::CFA_TEMPLATE_MODE ||
        sasInfo_.perfMode == optiling::SASTemplateMode::SCFA_TEMPLATE_MODE) {
        OP_CHECK_IF(cmpRatio_ != 128 && cmpRatio_ != 4,
                    OP_LOGE(opName_, "cmp_ratio should be 128 or 4, but got %u", cmpRatio_), return ge::GRAPH_FAILED);
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SASTilingCheck::CheckSinglePara() const
{
    if (ge::GRAPH_SUCCESS != CheckSingleParaQuery() || ge::GRAPH_SUCCESS != CheckSingleParaOriKv() ||
        ge::GRAPH_SUCCESS != CheckSingleParaCmpKv() || ge::GRAPH_SUCCESS != CheckSingleParaNumHeads() ||
        ...
        ge::GRAPH_SUCCESS != CheckSingleParaMetadata() || ge::GRAPH_SUCCESS != CheckSingleParaCmpRatio() ||
        ge::GRAPH_SUCCESS != CheckSingleParaOriMaskMode() || ge::GRAPH_SUCCESS != CheckSingleParaCmpMaskMode() ||
        ge::GRAPH_SUCCESS != CheckSingleParaOriKvStride() || ge::GRAPH_SUCCESS != CheckSingleParaCmpKvStride() ||
        ge::GRAPH_SUCCESS != CheckSingleParaOriWinLeft() || ge::GRAPH_SUCCESS != CheckSingleParaOriWinRight()) {
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus SASTilingCheck::CheckFeatureShape() const
{
    ...
    OP_CHECK_IF(gSize_ % 4 != 0, OP_LOGE(opName_, "group num should be multiple of 4, but got %u", gSize_),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(opName_, "Head dimension data type check failed! qType[%s] must be the same with oriKvType[%s].",
                SASDataTypeToSerialString(qType_).c_str(), SASDataTypeToSerialString(oriKvType_).c_str()),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(*opParamInfo_.cmpMaskMode != 3,
                ...
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(oriWinLeft_ < 0, OP_LOGE(opName_, "ori_win_left should be non-negative, but got %ld", oriWinLeft_),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(oriWinRight_ < 0, OP_LOGE(opName_, "ori_win_right should be non-negative, but got %ld", oriWinRight_),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
{
    OP_CHECK_IF(
        qType_ != ge::DT_BF16 && qType_ != ge::DT_FLOAT16,
        OP_LOGE(opName_, "q dtype only support %s and %s, but got %s", SASDataTypeToSerialString(ge::DT_BF16).c_str(),
                SASDataTypeToSerialString(ge::DT_FLOAT16).c_str(), SASDataTypeToSerialString(qType_).c_str()),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SASTilingCheck::CheckDTypeConsistency(const ge::DataType &actualDtype, const ge::DataType &expectDtype,
                                                      const std::string &name) const
{
    if (actualDtype != expectDtype) {
        OP_LOGE(opName_, "%s dtype should be the same to %s, but it's %s.", name.c_str(),
                SASDataTypeToSerialString(expectDtype).c_str(), SASDataTypeToSerialString(actualDtype).c_str());
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SASTilingCheck::CheckOriAndCmpKv() const
{
    OP_CHECK_IF(opParamInfo_.oriKv.tensor->GetStorageShape().GetShapeSize() == 0,
                OP_LOGE(opName_, "ori_kv cannot be empty tensor."), return ge::GRAPH_FAILED);
    if (sasInfo_.perfMode == SASTemplateMode::CFA_TEMPLATE_MODE ||
        sasInfo_.perfMode == SASTemplateMode::SCFA_TEMPLATE_MODE) {
        if (opParamInfo_.cmpKv.tensor->GetStorageShape().GetDim(0) != 0) {
            OP_CHECK_IF(opParamInfo_.cmpKv.tensor->GetStorageShape().GetShapeSize() == 0,
                        OP_LOGE(opName_, "cmp_kv cannot be empty tensor."), return ge::GRAPH_FAILED);
        }
...
ge::graphStatus SASTilingCheck::CheckActualSeqLensQ() const
{
    if (qLayout_ == SASLayout::TND) {
        OP_CHECK_IF(opParamInfo_.cuSeqLensQ.tensor->GetStorageShape().GetShapeSize() == 0,
                    OP_LOGE(opName_, "when q's is TND, cu_seqlens_q cannot be empty tensor."), return ge::GRAPH_FAILED);
...
void SparseAttnSharedkvTiling::CalcUbBmm(SASTilingInfo *tilingInfo)
{
    ...
    mmResUbSize_ =
        sInnerSizeAlign_ *
        Align(cubeMSize, 16U);  // The kernel writes with 16-element alignment; tiling allocates memory accordingly.
    bmm2ResUbSize_ =
        headDimAlign_ *
        Align(cubeMSize, 16U);  // The kernel writes with 16-element alignment; tiling allocates memory accordingly.
}
...
void SparseAttnSharedkvTiling::SplitBalanced(SASTilingInfo *tilingInfo)
{
    ...
    tilingData_.baseParams.mBaseSize = mBaseSize_;
    tilingData_.baseParams.s2BaseSize = sInnerSize_;
    tilingData_.baseParams.mmResUbSize = mmResUbSize_;
...
ge::graphStatus SparseAttnSharedkvTiling::DoOpTiling(SASTilingInfo *tilingInfo)
{
    ...
    if (tilingInfo->perfMode == SASTemplateMode::SCFA_TEMPLATE_MODE) {
        ...
        workspaceSize +=
            4 * 128 * 4 *
            (2 * aicNum);  // 4: cached valid MTE2-size length; 128: partitions; 4: 512-B alignment units; 2: AIV count.
    }
    workspaceSize_ = workspaceSize;
    context_->SetWorkspaceSizes(static_cast<size_t>(workspaceSize_));

    // -------------set tilingdata-----------------
    tilingData_.baseParams.batchSize = tilingInfo->bSize;
    tilingData_.baseParams.kvSeqSize = tilingInfo->s2Size;
    tilingData_.baseParams.qSeqSize = tilingInfo->s1Size;
    tilingData_.baseParams.nNumOfQInOneGroup = tilingInfo->gSize;
    tilingData_.baseParams.paBlockSize = tilingInfo->blockSize;
    tilingData_.baseParams.oriBlockSize = tilingInfo->oriBlockSize;
    tilingData_.baseParams.cmpBlockSize = tilingInfo->cmpBlockSize;
    tilingData_.baseParams.oriMaxBlockNumPerBatch = tilingInfo->oriMaxBlockNumPerBatch;
    tilingData_.baseParams.actualLenDimsQ = tilingInfo->actualLenDimsQ;
    tilingData_.baseParams.actualLenDimsKV = tilingInfo->actualLenDimsKV;

    tilingData_.baseParams.softmaxScale = tilingInfo->softmaxScale;
    tilingData_.baseParams.outputLayout = static_cast<uint32_t>(tilingInfo->outLayout);
    tilingData_.baseParams.oriMaskMode = tilingInfo->oriMaskMode;
    tilingData_.baseParams.oriKvStride = tilingInfo->oriKvStride;
    tilingData_.baseParams.oriWinLeft = tilingInfo->oriWinLeft;
    tilingData_.baseParams.oriWinRight = tilingInfo->oriWinRight;
    tilingData_.baseParams.sparseBlockSize = tilingInfo->sparseBlockSize;
    tilingData_.baseParams.hasOriSparseIndices = tilingInfo->hasOriSparseIndices;
    tilingData_.baseParams.oriSparseIndexWidth = tilingInfo->oriSparseIndexWidth;
    tilingData_.baseParams.returnSoftmaxLse = tilingInfo->returnSoftmaxLse;

    tilingData_.cmpParams.cmpMaxBlockNumPerBatch = tilingInfo->cmpMaxBlockNumPerBatch;
    tilingData_.cmpParams.sparseBlockCount = tilingInfo->sparseBlockCount;
    tilingData_.cmpParams.cmpRatio = tilingInfo->cmpRatio;
    tilingData_.cmpParams.cmpMaskMode = tilingInfo->cmpMaskMode;
    tilingData_.cmpParams.cmpKvStride = tilingInfo->cmpKvStride;

    usedCoreNum_ = aicNum;
    tilingData_.baseParams.usedCoreNum = usedCoreNum_;
    ...
    SASDType dtype = tilingInfo->qType == ge::DT_BF16 ? SASDType::BF16 : SASDType::FP16;
    tilingData_.dispatchKey =
        MakeSASDispatchKey(dtype, qLayout, inputKvLayout, static_cast<uint32_t>(tilingInfo->perfMode));
