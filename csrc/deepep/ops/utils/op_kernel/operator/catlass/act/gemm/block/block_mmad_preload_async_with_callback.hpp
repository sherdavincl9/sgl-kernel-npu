#ifndef ACT_GEMM_BLOCK_BLOCK_MMAD_PRELOAD_ASYNC_WITH_CALLBACK_HPP
...
public:
    ...
    static_assert(L1TileShape::M == L0TileShape::M && L1TileShape::N == L0TileShape::N,
                  "The situation where the basic blocks of L1 and L0 differ on "
                  "the m and n axes is not supported yet");
                    ...
                    Callback const &callbackAfterFixpipe)
    {
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; ++kLoopIdx) {
            ...
            // If the number of preload instructions reaches the upper limit, perform
            // an mmad calculation on L1 tile
        ...
        for (uint32_t mPartIdx = 0; mPartIdx < mPartLoop; ++mPartIdx) {
            ...
            for (uint32_t kPartIdx = 0; kPartIdx < kPartLoop; ++kPartIdx) {
                ...
                for (uint32_t nPartIdx = 0; nPartIdx < nPartLoop; ++nPartIdx) {
                    ...
                    // If the current tile is the first tile on the k axis, the
                    // accumulator needs to be reset to 0
...
#endif  // ACT_GEMM_BLOCK_BLOCK_MMAD_PRELOAD_ASYNC_WITH_CALLBACK_HPP
