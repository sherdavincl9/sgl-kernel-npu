#ifndef ACT_EPILOGUE_TILE_TILE_BROADCAST_MUL_HPP
#define ACT_EPILOGUE_TILE_TILE_BROADCAST_MUL_HPP

#include "../../../act/act.hpp"

namespace Act::Epilogue::Tile {

/// BroadcastMul computes the elementwise multiplication of a tensor of shape
/// (m, n) and a tensor of shape (m, n) after broadcasting. There are two
/// broadcast modes: row-broadcast and column-broadcast.

/// @brief Computes the elementwise multiplication of a tensor with shape (m, n)
/// and a tensor with original shape (1, n) broadcast to (m, n).
...
/// @brief Compute the elementwise multiplication of a tensor of shape (m, n)
/// and a tensor of shape (m, eleNumPerBlk), which is broadcast from a tensor of
/// shape (m, 1), broadcast to (m, n).
/// @tparam ArchTag_ is the architecture tag.
/// @tparam ComputeType_ includes the element type and layout information.
/// @tparam TileShape_ is the shape (m, n).
template <class ArchTag_, class ComputeType_, class TileShape_>
