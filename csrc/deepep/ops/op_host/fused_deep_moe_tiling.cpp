...
#define OPS_UTILS_LOG_PACKAGE_TYPE "DEEPEP_OPS"
#endif
...
public:
    static uint64_t GetMaxWindowSize(const char *nodeName)
    {
        ...
        OPS_LOG_I(nodeName, "Get maxWindowSize is %lu", maxWindowSize);
                                        ...
                                        FusedDeepMoeTilingData &tilingData)
{
    ...
    OPS_ERR_IF(gmm1WeightDim0 != localExpertNum,
               OPS_LOG_E(nodeName, "gmm1Weight Dim0 must be expert number in current rank."), return ge::GRAPH_FAILED);
    ...
    OPS_ERR_IF(gmm2WeightDim0 != localExpertNum,
               OPS_LOG_E(nodeName, "gmm2Weight Dim0 must be expert number in current rank."), return ge::GRAPH_FAILED);
...
static ge::graphStatus FusedDeepMoeTilingFuncImpl(gert::TilingContext *context)
{
    ...
    uint64_t maxWindowSize = Mc2TilingUtils::GetMaxWindowSize(nodeName);
