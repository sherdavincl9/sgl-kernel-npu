...
template <class ArchTag_, bool ENABLE_UNIT_FLAG_, uint32_t L1_SCALE_FACTOR_K_, uint32_t L0C_STAGES_,
          bool ENABLE_L1_RESIDENT_, uint32_t L1A_STAGES_, uint32_t L1B_STAGES_, uint32_t L0A_STAGES_,
...
struct BlockMmadTla<MmadMxWithCallback<ArchTag_, ENABLE_UNIT_FLAG_, L1_SCALE_FACTOR_K_, L0C_STAGES_,
...
public:
    // Type Aliases
    using DispatchPolicy = MmadMxWithCallback<ArchTag_, ENABLE_UNIT_FLAG_, L1_SCALE_FACTOR_K_, L0C_STAGES_,
                                              ENABLE_L1_RESIDENT_, L1A_STAGES_, L1B_STAGES_, L0A_STAGES_, L0B_STAGES_>;
    ...
    template <class TensorA, class TensorB, class TensorC, class TensorMxScaleA = EmptyClass,
        ...
        for (uint32_t kL1Idx = 0; kL1Idx < kL1Loop; kL1Idx++) {
            ...
            if (kL1Idx < kL1Loop - 1) {
                ...
                if (kL1IdxNext % L1_SCALE_FACTOR_K == 0) {
                    ...
                    auto tensorL1MxScaleA_pre = tla::MakeTensor(l1MxScaleATensorList[l1MxScaleAListIdNext],
                                                                L1SCALEA_LAYOUT, Arch::PositionL1{});
                    auto tensorTileMxScaleA =
                        GetTile(tensorMxScaleA, tla::MakeCoord(0, kL1ScaleIdxNext * l1ScaleTileK / MX_SCALE_GROUP_NUM),
                ...
                if (kL1IdxNext % L1_SCALE_FACTOR_K == 0) {
                    ...
                    uint32_t kL1MxScaleActualNext = (kL1ScaleIdxNext < kL1ScaleLoop - 1)
                                                        ? l1ScaleTileK
                                                        : (kBlockActual - kL1ScaleIdxNext * l1ScaleTileK);
                    auto tensorL1MxScaleB_pre = tla::MakeTensor(l1MxScaleBTensorList[l1MxScaleBListIdNext],
                                                                L1SCALEB_LAYOUT, Arch::PositionL1{});
                    auto tensorTileMxScaleB =
                        GetTile(tensorMxScaleB, tla::MakeCoord(kL1ScaleIdxNext * l1ScaleTileK / MX_SCALE_GROUP_NUM, 0),
