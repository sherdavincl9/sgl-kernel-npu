...
def _causal_conv1d_linear_verify_kernel(
    ...
    state_base = (
        state_ptr + state_idx * stride_state_slot + channel * stride_state_channel
    )
    ...
    if KERNEL_WIDTH >= 2:
        history0 = tl.load(state_base, mask=mask, other=0.0).to(tl.float32)
    if KERNEL_WIDTH >= 3:
        history1 = tl.load(state_base + stride_state_window, mask=mask, other=0.0).to(
            tl.float32
        )
    if KERNEL_WIDTH >= 4:
        history2 = tl.load(
    ...
    if HAS_BIAS:
        bias = tl.load(bias_ptr + channel, mask=channel_mask, other=0.0).to(tl.float32)
    else:
...
def causal_conv1d_linear_verify_npu(
    ...
    if intermediate_conv_window.ndim != 4 or tuple(
        ...
        raise ValueError("intermediate_conv_window must have shape [scratch, T, C, W]")
